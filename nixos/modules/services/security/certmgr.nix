...
with lib; let
  ...
  specPaths = map dirOf (concatMap (
    spec:
      if isAttrs spec
      then collect isString (filterAttrsRecursive (n: v: isAttrs v || n == "path") spec)
      else [spec]
      ...
      type = with types;
        attrsOf (either path (submodule {
          options = {
            ...
            ca = mkOption {
              type = nullOr attrs;
              description = lib.mdDoc "certmgr spec ca object.";
            };

            key_usages = mkOption {
              type = nullOr (listOf str);
              description = lib.mdDoc "certmgr spec key_usages object.";
            };
          };
        }));
